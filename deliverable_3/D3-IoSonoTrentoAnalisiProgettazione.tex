...
Al primo accesso Google, se \texttt{profiloCompleto = false}, il cittadino deve fornire nome,
cognome, codice fiscale e data di nascita tramite \texttt{POST /auth/complete-profile}.
...
Dati anagrafici del cittadino (nome, cognome, codice fiscale, data di nascita) necessari per
portare \texttt{profiloCompleto} a \texttt{true}.
...
propri dati, il completamento del profilo al primo accesso e il logout. Per gli operatori espone
funzionalità analoghe: visualizzazione del profilo, cambio password e promozione a \textit{root}.

\interfacciarichiesta{autenticazione}\\
JWT valido per garantire che ogni utente acceda solo al proprio profilo.

\interfacciarichiesta{dati completamento profilo}\\
Nome, cognome, codice fiscale e data di nascita, forniti dal cittadino al primo accesso.
...
\begin{itemize}
  ...
  \item \texttt{codiceFiscale}: codice fiscale italiano
...
\begin{itemize}
  \item \texttt{completeProfile(dati)}: imposta i campi anagrafici e porta
        \texttt{profiloCompleto} a \texttt{true}
