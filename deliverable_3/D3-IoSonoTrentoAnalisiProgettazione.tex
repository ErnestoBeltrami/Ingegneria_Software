...
Nel presente capitolo viene presentata l'architettura in termini di componenti (CMP) interni al
sistema, definiti sulla base dei requisiti analizzati in precedenza. Viene poi adottato il diagramma
dei componenti per rappresentare l'interconnessione tra i vari moduli, identificando le interfacce
tra questi e verso sistemi esterni.
...
nascita, comune di residenza e circoscrizione (se residente a Trento) tramite
...
Username, password in chiaro e flag \texttt{isRoot}. Il componente salva la password come hash
bcrypt.

\interfacciarichiesta{dati profilo cittadino}\\
Dati anagrafici del cittadino (nome, cognome, data di nascita, comune di residenza, circoscrizione)
necessari per portare \texttt{profiloCompleto} a \texttt{true} o per aggiornare il profilo.

\interfacciarichiesta{dati consultazione}\\
Tipo (\texttt{votazione} | \texttt{sondaggio}), titolo, date di inizio, fine e discussione,
domande associate, stato iniziale (\texttt{bozza}).
...
lo scheduler interno le porta automaticamente allo stato \texttt{attivo} alla \texttt{data\_inizio}
e a \texttt{concluso} alla \texttt{data\_fine}. I cittadini possono rispondere alle consultazioni
attive.
...
Aggregazione del numero di voti per opzione, suddivisi fasce d'età dei votanti, fornita agli
operatori al termine della consultazione.
...
Data di nascita, comune di residenza e circoscrizione (se residente a Trento), forniti dal
cittadino tramite \texttt{POST /auth/complete-profile}.
...
visualizzabili dagli operatori. In particolare, per ogni votazione conclusa, calcola la
distribuzione dei voti per opzione e per fascia d'età (derivata dalla data di nascita del
cittadino). Espone queste aggregazioni tramite le API di reportistica.
...
JSON con il conteggio voti per opzione e la distribuzione per fasce d'età
(\texttt{18-30}, \texttt{31-50}, \texttt{51-70}, \texttt{71+}).
...
$\texttt{attivo} \to \texttt{concluso}$ quando \texttt{data\_fine} è trascorsa.
...
\begin{itemize}
  ...
  \item \texttt{comuneResidenza}: comune di residenza del cittadino
  \item \texttt{circoscrizione}: circoscrizione di Trento (valorizzata solo se residente a Trento)
...
\begin{itemize}
  \item \texttt{completeProfile(dati)}: imposta \texttt{dataNascita}, \texttt{comuneResidenza} e
        \texttt{circoscrizione}; porta \texttt{profiloCompleto} a \texttt{true}
...
\begin{itemize}
  ...
  \item \texttt{register(username, password)}: crea un nuovo account operatore
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{titolo}: titolo della consultazione
...
\begin{itemize}
  ...
  \item \texttt{opzioni}: array di stringhe con le risposte possibili
...
\begin{itemize}
  ...
  \item \texttt{risposte}: mappa domanda $\to$ opzioni selezionate
  \item \texttt{dataRisposta}: timestamp della risposta
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{dataVoto}: timestamp del voto
\end{itemize}
...
\noindent\textbf{Attributi principali:}
\begin{itemize}
  \item \texttt{nome}: nome della categoria
  \item \texttt{descrizione}: breve descrizione della tematica
\end{itemize}
...
\begin{longtable}{>{\ttfamily\small}p{0.38\textwidth} p{0.14\textwidth} >{\ttfamily\small}p{0.38\textwidth}}
  ...
  Consultazione.getById() (votazione, op.) & GET & /votazioni/:id \\
  Consultazione.getById() (votazione, cit.) & GET & /votazioni/:id \textsuperscript{†} \\
...
\noindent\textsuperscript{‡} \texttt{POST /cittadino/vote/votazione} verifica che la votazione sia
in stato \texttt{attivo} prima di accettare il voto; restituisce \texttt{403} se lo stato non è
\texttt{attivo} o se il cittadino ha già votato.
...
\texttt{validateObjectId} previene \texttt{CastError} di Mongoose sui parametri \texttt{/:id}.
