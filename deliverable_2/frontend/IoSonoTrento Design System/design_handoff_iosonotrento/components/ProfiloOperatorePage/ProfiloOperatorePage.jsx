 ...
 *
 * Pagina profilo per IoSonoTrento (app civica del Comune di Trento).
 * Layout: sidebar identità a sinistra, info account + cambio password a destra.
 *
 * Props:
 *   user      { nome, cognome, username, ruolo: 'operatore' | 'root' }
 *   onBack    () => void          // back link in alto
 *   onSubmit  ({ passwordAttuale, nuovaPassword }) => Promise<void>
 *                                 // se omesso, mostra solo un mock locale
 *
...
export default function ProfiloOperatorePage({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      if (onSubmit) {
        await onSubmit({ passwordAttuale, nuovaPassword });
      } else {
  ...
  return (
    <div className="po-page">
      <div className="po-shell">
        ...
        {onBack && (
          <button type="button" className="po-back" onClick={onBack}>
