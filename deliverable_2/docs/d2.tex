...
organizzati nella cartella \code{backend/tests/unit/}, con un file per ciascun controller o
modulo principale. Complessivamente sono definiti \textbf{114 casi di test} così distribuiti:

\begin{itemize}
	\item \code{categoria.controller.test.js} --- 16 test;
	\item \code{cittadino.test.js} --- 27 test;
	\item \code{consultazione.controller.test.js} --- 10 test;
	\item \code{iniziativa.controller.test.js} --- 4 test;
	\item \code{notifica.controller.test.js} --- 9 test;
	\item \code{operatore.controller.test.js} --- 24 test;
	\item \code{seedRoot.test.js} --- 3 test;
	\item \code{sondaggio.controller.test.js} --- 13 test;
...
L'applicazione è pubblicata su Render in un monorepo con due servizi distinti,
con deploy automatico ad ogni commit sul branch \code{main} tramite integrazione
nativa con GitHub.

\begin{itemize}
  \item Backend (Web Service): \url{https://ingegneria-software.onrender.com} —
        root directory \texttt{deliverable\_2/backend},
        build command \texttt{npm install},
        start command \texttt{npm run dev}
  \item Frontend (Static Site): \url{https://ingegneria-software-7gs8.onrender.com} —
        root directory \texttt{deliverable\_2/frontend},
        build command \texttt{npm install \&\& npm run build},
        publish directory \texttt{dist}
  \item Account dimostrativo operatore: username \texttt{root}, password \texttt{IoSonoTrento123!}
  \item \textbf{Nota:} il backend è ospitato sul piano gratuito di Render; dopo un periodo
        di inattività il servizio va in sleep e la prima richiesta può richiedere fino a
        30--60 secondi prima di rispondere.
\end{itemize}
