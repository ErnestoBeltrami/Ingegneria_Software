...
\begin{itemize}
    \item \textbf{RF14 - Creazione account operatore}: Il sistema deve consentire a qualsiasi operatore autenticato di creare nuovi account operatore (\texttt{POST /operatore/register}), inserendo username, password, nome e cognome. Al primo avvio il sistema crea automaticamente un account root predefinito (\textit{seedRoot}) con credenziali iniziali note, non eliminabile.
    \item \textbf{RF15 - Promozione a root}: Il sistema deve consentire all’account root di promuovere un operatore esistente al ruolo root tramite l’endpoint \texttt{PATCH /operatore/:id/promote}. Solo un operatore con flag \texttt{isRoot: true} può eseguire questa operazione.
    \item \textbf{RF16 - Disattivazione account amministratore} \textit{(requisito futuro)}: In una versione futura il sistema dovrà consentire all’account root di disattivare account operatore esistenti senza eliminarne i dati, registrando data e ora dell’operazione per finalità di audit.
\end{itemize}
