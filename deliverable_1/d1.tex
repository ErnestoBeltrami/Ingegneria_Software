...
\begin{itemize}
      ...
      \item \textbf{verifica dell’identità fisica}: L'autenticazione SPID garantisce l'identità digitale, ma non può impedire la cessione volontaria delle credenziali (es. un familiare che vota per conto di un altro).
\end{itemize}
...
\begin{itemize}
    \item \textbf{RF14 - Creazione account admin}: Il sistema deve prevedere la presenza di un account root preconfigurato e non eliminabile, utilizzato esclusivamente per la creazione e l’abilitazione degli account amministrativi destinati agli operatori del Comune. L’account root ha accesso unicamente alle funzionalità di creazione account amministratore, visualizzazione della lista amministratori e disattivazione degli account amministratore.
    \item \textbf{RF15 - Visualizzazione lista amministratori}: Il sistema deve consentire all’account root di visualizzare l’elenco completo degli account amministrativi registrati nel sistema. La lista deve includere almeno nome utente, stato account (attivo/disattivato), data di creazione ed eventuale data di disattivazione.
    \item \textbf{RF16 - Disattivazione account amministratore}: Il sistema deve consentire all’account root di disattivare uno o più account amministrativi esistenti senza eliminarne i dati. Il sistema deve impedire la disattivazione dell’account root stesso e registrare data e ora della disattivazione per finalità di audit.
\end{itemize}
...
\begin{enumerate}[label=\textbf{RNF\arabic*}, resume]
	...
	\item \textbf{Gestione sicura delle sessioni}: le sessioni devono scadere automaticamente dopo un periodo di inattività prestabilito, prevenendo accessi non autorizzati. Abbiamo deciso di impostare questo limite a 15 minuti di attivita per arrivare a un compromesso tra durata della sessione e sicurezza dei dati. 
...
\begin{enumerate}
    \item L'Account Root accede alla sezione di creazione account amministratore.
    \item Il sistema presenta un form per nome utente e password iniziale.
    \item L'Account Root inserisce i dati e conferma la creazione.
    \item Il sistema valida i dati, crea l'account in stato \textbf{attivo} e registra la data di creazione.
\end{enumerate}

\textbf{Eccezioni}
\begin{itemize}
    \item Dati incompleti: il sistema nega la creazione.
    \item Nome utente già esistente: il sistema nega la creazione.
\end{itemize}

% --------------------------------------------------------

\subsection{RF15: Visualizzazione lista amministratori}
\begin{enumerate}
    \item L'Account Root accede alla sezione \textit{Gestione Amministratori}.
    \item Il sistema recupera l'elenco completo degli account amministrativi.
    \item Il sistema mostra per ogni account: nome utente, stato, data di creazione ed eventuale data di disattivazione.
\end{enumerate}

% --------------------------------------------------------

\subsection{RF16: Disattivazione account amministratore}
\begin{enumerate}
    \item L'Account Root visualizza la lista degli account amministrativi.
    \item Seleziona uno o più account \textbf{attivi} da disattivare.
    \item Il sistema richiede conferma esplicita.
    \item Se confermata, il sistema imposta lo stato a \textbf{disattivato} e registra data/ora dell'operazione.
\end{enumerate}

\textbf{Eccezioni}
\begin{itemize}
    \item Account già disattivato: il sistema non esegue l'azione e mostra un messaggio.
    \item Tentativo di disattivazione dell'account root: il sistema blocca l'operazione.
\end{itemize}
...
\subsection{User Story 1: Gestione Accessi (UC RF1)}
